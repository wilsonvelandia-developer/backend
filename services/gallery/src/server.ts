...
async function main() {
  ...
  const server = app.listen(config.port, () => {
    logger.info({ port: config.port, env: config.nodeEnv }, 'Gallery service started');
  });
...
main().catch((err) => { console.error('Failed to start gallery service:', err); process.exit(1); });
