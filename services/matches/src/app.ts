...
export async function createApp() {
  ...
  app.get('/health', async (_req: Request, res: Response) => {
    try {
      ...
      res.json({ status: 'ok', service: 'matches', timestamp: new Date().toISOString() });
    } catch {
      res.status(503).json({ status: 'error', service: 'matches' });
    }
  });
  ...
  logger.info({ port: config.port, env: config.nodeEnv }, 'Matches service configured');
